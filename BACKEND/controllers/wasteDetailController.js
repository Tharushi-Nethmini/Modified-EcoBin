...
const { multipleWasteDetailsSchema, wasteDetailSchema } = require('../validation/wasteDetailValidation');

// Add multiple waste details
exports.addWasteMultiple = async (req, res) => {
    // security fix: validate input with Joi
    const { error } = multipleWasteDetailsSchema.validate(req.body);
...
exports.updateWaste = async (req, res) => {
    ...
    // security fix: validate input with Joi
    const { error } = wasteDetailSchema.validate(req.body);
    if (error) return res.status(400).json({ error: error.details[0].message });
