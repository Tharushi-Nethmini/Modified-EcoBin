...
const { compostRequestSchema, compostRequestUpdateSchema } = require('../validation/compostRequestValidation');

// Create a new compost request
exports.addCompostRequest = async (req, res) => {
    // security fix: validate input with Joi
    const { error } = compostRequestSchema.validate(req.body);
...
exports.updateCompostRequest = async (req, res) => {
    ...
    // security fix: validate input with Joi
    const { error } = compostRequestUpdateSchema.validate(req.body);
    if (error) return res.status(400).json({ error: error.details[0].message });
